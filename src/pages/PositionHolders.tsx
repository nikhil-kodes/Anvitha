      ...
      <div className="container mx-auto max-w-6xl">
        ...
        >
          ...
          <p className="text-xl text-glow-green/80 font-exo max-w-2xl mx-auto">
            Meet the brilliant minds leading Anvitha ECE Club into the future of technology
          </p>
        </motion.div>
        ...
        >
          {positions.map((position, index) => {
            ...
            return (
              ...
              >
                {/* Background Glow */}
                <div className={`absolute inset-0 bg-gradient-to-br from-${position.color}/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300`} />
                
                {/* Content */}
                <div className="relative z-10">
                  {/* Role Badge */}
                  <div className="flex items-center justify-between mb-4">
                    <div className={`w-12 h-12 bg-${position.color}/20 rounded-full flex items-center justify-center group-hover:animate-pulse`}>
                      <Icon className={`w-6 h-6 text-${position.color}`} />
                    </div>
                    <div className={`px-3 py-1 bg-${position.color}/20 rounded-full`}>
                      <span className={`text-${position.color} font-exo text-sm font-bold`}>
                        {position.role}
                      </span>
                    </div>
                  </div>
        ...
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              "Aditya Raj", "Meera Iyer", "Rohan Desai", "Kavya Nair",
              "Suresh Reddy", "Ananya Das", "Karthik Menon", "Divya Shah"
            ].map((name, index) => (
              <motion.div
                key={name}
