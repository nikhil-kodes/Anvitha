		...
		<div className="min-h-screen flex flex-col items-center justify-center px-4 sm:px-6 lg:px-8">
			<motion.div
				...
				<div className="mb-6 sm:mb-8">
					<motion.h1
						className="font-orbitron font-black text-3xl sm:text-4xl md:text-6xl lg:text-8xl text-white text-neon-cyan"
						initial={{ scale: 0.8 }}
						animate={{ scale: 1 }}
					...
					<motion.div
						className="flex flex-col sm:flex-row items-center justify-center sm:space-x-2 md:space-x-4 text-sm sm:text-lg md:text-2xl lg:text-3xl text-gray-300 font-exo font-light tracking-wide sm:tracking-widest space-y-2 sm:space-y-0"
						initial={{ opacity: 0 }}
						animate={{ opacity: 1 }}
						transition={{ delay: 0.5, duration: 1 }}>
						<div className="flex items-center space-x-2">
							<Zap className="w-4 h-4 sm:w-6 sm:h-6 text-fushia md:w-8 md:h-8" />
							<span>Innovation</span>
						</div>
						<div className="hidden sm:block w-2 h-2 bg-fushia rounded-full animate-glow-pulse" />
						<span>Legacy</span>
						<div className="hidden sm:block w-2 h-2 bg-fushia rounded-full animate-glow-pulse" />
						<div className="flex items-center space-x-2">
							<span>Technology</span>
							<Zap className="w-4 h-4 sm:w-6 sm:h-6 md:w-8 text-fushia md:h-8" />
						</div>
					</motion.div>
				</div>
				...
				{/* Description */}
				<motion.p
					className="text-sm sm:text-base md:text-lg lg:text-xl text-neutral-300 mb-8 sm:mb-12 leading-relaxed font-exo max-w-2xl mx-auto px-4 sm:px-0"
				...
				<motion.div
					...
					<div className="bg-gradient-to-r  from-black to-[#0a1647] border border-neutral-500  p-4 sm:p-6 group hover:scale-105 transition-transform duration-300 rounded-lg">
						<Target className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 text-cyber-green mx-auto mb-3 sm:mb-4 group-hover:animate-spin" />
						<h3 className="font-orbitron font-bold text-lg sm:text-xl text-white mb-2">
							Innovation Hub
						</h3>
						<p className="text-neutral-300 font-exo text-sm sm:text-base">
							Fostering creativity and breakthrough solutions in electronics
						</p>
					</div>

					<div className="bg-gradient-to-r  from-black to-[#0a1647] border  border-neutral-500 shadow-[0_0_25px_rgba(255,255,255,0.2)] p-4 sm:p-6 group hover:scale-105 transition-transform duration-300 rounded-lg">
					...
					<div className="bg-gradient-to-r from-black to-[#0a1647] border  border-neutral-500  p-4 sm:p-6 group hover:scale-105 transition-transform duration-300 rounded-lg sm:col-span-2 lg:col-span-1">
							...
							<div className="relative max-w-xs sm:max-w-sm md:max-w-md mx-auto my-6 sm:my-8 md:my-10">
								...
								{anvitha.map((letter, index) => (
									<motion.div
										...
										<div className="absolute rounded-full size-3 sm:size-4 bg-teal-500 left-1/2 transform -translate-x-1/2 z-10">
											<div className="absolute inset-0 bg-teal-600 rounded-full animate-ping" />
										</div>
										<div className="absolute w-1/2 h-[1px] bg-white -z-10"></div>
										<div className="bg-white border-2 sm:border-4 flex flex-col items-center justify-center border-blue-600 p-1.5 sm:p-2 size-16 sm:size-20 md:size-28 rounded-lg hover:shadow-[0_0_10px_#87CEEB] transition-all duration-300">
											<h3 className="text-lg sm:text-2xl md:text-4xl text-sky-600 font-bold font-orbitron">
												{letter.initial}
											</h3>
				...
				<motion.div
					...
					<div className="flex flex-col lg:flex-row gap-6 sm:gap-8 items-center backdrop-blur-sm bg-neutral-300/10 rounded-xl p-4 lg:items-start justify-center">
						<img
							src={hod}
							alt="Head of Department"
							className="border sm:border-2 h-48 sm:h-56 md:h-60 lg:h-72 w-auto border-neutral-300 rounded-xl flex-shrink-0"
						/>
						<p className="text-xs sm:text-sm md:text-base text-neutral-300 leading-relaxed text-justify lg:text-left max-w-2xl">
